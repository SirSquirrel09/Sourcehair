...
function UpdateFiles() {
    ...
    //deletes UpdaterSrc and updater.js from the downloaded version
    ...
    //Installs new files to the "src" directory
    setTimeout(() => {
        //old code. but im gonna keep it here
        //fs.readdirSync(UpdateFolderDir+"/src").forEach((file) => {
        ...
        fs.readdirSync(UpdateFolderDir+"/src").forEach((file) => { //New
            let oldPath = UpdateFolderDir+"/src/"+file
            let newPath = path.join(__dirname, file)
    ...
    setTimeout(() => {
        //Removes updated version and launches
